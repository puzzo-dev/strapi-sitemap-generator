...
interface IndustriesCTASectionProps extends PageSection {
  isLoading?: boolean;
}

const IndustriesCTASection: React.FC<IndustriesCTASectionProps> = ({
  title,
  subtitle,
  content,
  settings,
      ...
      <div className="container-custom max-w-7xl">
        <div className="text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700 dark:bg-blue-900/30 dark:border-blue-800 dark:text-blue-300 mb-4 animate-fade-in">
            {settings?.badge || 'Get Started'}
          </div>
          <h2 className="section-title text-blue-900 dark:text-blue-200">
            {title || 'Ready to Transform Your Industry?'}
          </h2>
          <p className="section-subtitle mb-8">
            {subtitle || 'Contact our team today to learn more about our industry-specific solutions and how we can help your business succeed in the digital age.'}
          </p>
          <GradientButton href="/contact" size="lg">
            {settings?.primaryButton?.children || 'Get in Touch'}
          </GradientButton>
        </div>
      </div>
    </section>
